...
class SettingController extends RootPartnerController
{
    ...
    public function index()
    {
       ...
       // $store_id = session('partnerId')??'';
        ...
        $data['partnerId']                        = $id;
        $data['urlUpdateConfig']                = au_route_partner('partner_config.update');
        $data['urlUpdateConfigGlobal']          = au_route_partner('partner_config_global.update');
        $data['store']=$store;
        return view($this->templatePathPartner.'screen.setting.config',$data);
    }
