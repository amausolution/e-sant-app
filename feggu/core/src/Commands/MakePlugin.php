...
class MakePlugin extends Command
{
     ...
     * @var string
     */
    protected $signature = 'au:make {function} {--name=} {--download=0}';
    ...
    public function plugin($code = 'Other', $key = '', $download = 0)
    {
        ...
        $arrcodePlugin = ['Cms', 'Other', 'Payment', 'Shipping', 'Total','Laboratory'];
        $pluginKey = au_word_format_class($key);
        $pluginCode = au_word_format_class($code);
        if (!in_array($pluginCode, $arrcodePlugin)) {
            $pluginCode = 'Other';
        }
        $pluginUrlKey = au_word_format_url($key);
        ...
        try {
            File::copyDirectory(base_path('vendor/feggu/core/src/'.$source), storage_path($tmp));
            File::copyDirectory(base_path('vendor/feggu/core/src/'.$sourcePublic), storage_path($tmpPublic));
            ...
            $frontController = file_get_contents(storage_path($tmp.'/Controllers/PartnerController.php'));
            $frontController      = str_replace('Plugin_Code', $pluginCode, $frontController);
            $frontController      = str_replace('Plugin_Key', $pluginKey, $frontController);
            $frontController      = str_replace('PluginUrlKey', $pluginUrlKey, $frontController);
            file_put_contents(storage_path($tmp.'/Controllers/PartnerController.php'), $frontController);
            ...
            $langvi = file_get_contents(storage_path($tmp.'/Lang/fr/lang.php'));
            $langvi      = str_replace('Plugin_Code', $pluginCode, $langvi);
            $langvi      = str_replace('Plugin_Key', $pluginKey, $langvi);
            file_put_contents(storage_path($tmp.'/Lang/fr/lang.php'), $langvi);
        ...
        try {
            if ($download) {
                au_zip(storage_path($this->tmpFolder."/".$sID), storage_path($this->tmpFolder.'/'.$sID.'.zip'));
                $path = $sID;
            } else {
