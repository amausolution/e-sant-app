...
class Backup extends Command
{
     ...
     * @var string
     */
    protected $signature = 'au:backup {--path=} {--includeTables=} {--excludeTables=}';
     ...
     * @var string
     */
    protected $description = 'Backup database "au:backup --path=abc.sql --includeTables=table1,table2 --excludeTables=t1,t2"';
    const LIMIT = 10;
    ...
    public function handle()
    {
        ...
        if (count(glob(storage_path() . "/backups/*.sql")) >= self::LIMIT) {
            echo json_encode(['error' => 1, 'msg' => au_language_render('admin.backup.limit_backup')]);
            exit;
        }
        if ($path) {
        ...
        try {
            $databaseName = config('database.connections.'.AU_CONNECTION.'.database');
            $userName = config('database.connections.'.AU_CONNECTION.'.username');
            $password = config('database.connections.'.AU_CONNECTION.'.password');
            $host = config('database.connections.'.AU_CONNECTION.'.host');
            $pathMysqlBin = config('database.connections.'.AU_CONNECTION.'.path_mysql_bin');// C:\xampp\mysql\bin
