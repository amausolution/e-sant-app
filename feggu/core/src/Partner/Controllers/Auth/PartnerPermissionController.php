...
class PartnerPermissionController extends RootPartnerController
{
    public $routeAdmin;
    ...
    public function __construct()
    {
        ...
        foreach ($routes as $route) {
            if (Str::startsWith($route->uri(), AU_ADMIN_PREFIX)) {
                $prefix = AU_ADMIN_PREFIX?$route->getPrefix():ltrim($route->getPrefix(), '/');
                $routeAdmin[$prefix] = [
                    'uri'    => 'ANY::' . $prefix . '/*',
                    'name'   => $prefix . '/*',
                    'method' => 'ANY',
    ...
    public function index()
    {
        $data = [
            ...
            'subTitle' => '',
            'icon' => 'fa fa-indent',
            'urlDeleteItem' => au_route_partner('admin_permission.delete'),
        ...
        foreach ($dataTmp as $key => $row) {
            ...
            $dataTr[] = [
                ...
                'action' => '
                    <a href="' . au_route_partner('admin_permission.edit', ['id' => $row['id']]) . '"><span title="' . au_language_render('action.edit') . '" type="button" class="btn btn-flat btn-sm btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
        ...
        //menuRight
        $data['menuRight'][] = '<a href="' . au_route_partner('admin_permission.create') . '" class="btn  btn-success  btn-flat" title="New" id="button_create_new">
        ...
        $data['urlSort'] = au_route_partner('admin_permission.index', request()->except(['_token', '_pjax', 'sort_order']));
    ...
    public function create()
    {
        $data = [
            ...
            'url_action' => au_route_partner('admin_permission.create'),
    ...
    public function postCreate()
    {
        ...
        return redirect()->route('admin_permission.index')->with('success', au_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        $data = [
            ...
            'routeAdmin' => $this->routeAdmin,
            'url_action' => au_route_partner('admin_permission.edit', ['id' => $permission['id']]),
        ];
        return view($this->templatePathAdmin.'auth.permission')
            ->with($data);
    }
...
//
        return redirect()->route('admin_permission.index')->with('success', au_language_render('action.edit_success'));
    }
    ...
    {
        $prefix = AU_ADMIN_PREFIX?AU_ADMIN_PREFIX.'/':'';
        return [
            $prefix . 'login',
            $prefix . 'logout',
            $prefix . 'forgot',
