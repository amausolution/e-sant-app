...
class PartnerConfig extends Model
{
    ...
    public $table = AU_DB_PREFIX.'partner_config';
    protected $guarded = [];

    protected static $getAll = null;
    protected static $getAllGlobal = null;
    protected static $getAllConfigOfPartner = null;
    ...
    {
        if (self::$getAll === null) {
            self::$getAll = self::where('partner_id', '<>', 0)
                ->get()
                ->keyBy('partner_id');
        }
        return self::$getAll;
    ...
    public static function getAllGlobal():array
    {
        if (self::$getAllGlobal === null) {
            self::$getAllGlobal = self::where('partner_id', 0)
                ->pluck('value', 'key')
                ->all();
        }
    ...
    /**
     * [getAllConfigOfPartner description]
     *
     * @param   [type]  $partnerId  [$partnerId description]
     *
     * @return  [type]            [return description]
     */
    public static function getAllConfigOfPartner($partnerId):array
    {
        if (self::$getAllConfigOfPartner === null) {
            self::$getAllConfigOfPartner = self::get()
                ->groupBy('partner_id');
        }
    ...
    public static function getListConfigByCode(array $dataQuery)
    {
        ...
        $partnerId = $dataQuery['partnerId'] ?? 0;
