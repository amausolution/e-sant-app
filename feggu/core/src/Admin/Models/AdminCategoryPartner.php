...
use Feggu\Core\Partner\Models\FegguCategoryPartnerDescription;
use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;
use phpDocumentor\Reflection\Types\Self_;

class AdminCategoryPartner extends FegguCategoryPartner
{
    ...
    public static function getCategoryListAdmin(array $dataSearch)
    {
        ...
        $tableCategory     = (new FegguCategoryPartner)->getTable();

        $categoryList = (new FegguCategoryPartner)
    ...
    public static function getListTitleAdmin()
    {
        ...
        $tableDescription = (new FegguCategoryPartnerDescription)->getTable();
        $table = (new AdminCategoryPartner)->getTable();
        if (au_config_global('cache_status') && au_config_global('cache_category')) {
            if (!Cache::has($storeCache.'_cache_category_'.au_get_locale())) {
                ...
                au_set_cache($storeCache.'_cache_category_'.au_get_locale(), self::$getListTitleAdmin);
            }
            return Cache::get($storeCache.'_cache_category_'.au_get_locale());
        } else {
            if (self::$getListTitleAdmin === null) {
                self::$getListTitleAdmin = self::join($tableDescription, $tableDescription.'.category_id', $table.'.id')
                    ->where('lang', au_get_locale())
    ...
    {
        return FegguCategoryPartnerDescription::create($dataInsert);
    }
}
