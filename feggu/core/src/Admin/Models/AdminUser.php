...
class AdminUser extends Authenticatable
{
    use  Notifiable;
    //use HasApiTokens;
    public $table      = AU_DB_PREFIX.'admin_user';
    protected $guarded = [];
    protected $hidden  = [
    ...
    {
        return $this->belongsToMany(AdminRole::class, AU_DB_PREFIX.'admin_role_user', 'user_id', 'role_id');
    }
    ...
    {
        return $this->belongsToMany(AdminPermission::class, AU_DB_PREFIX.'admin_user_permission', 'user_id', 'permission_id');
    }
    ...
    protected static function boot()
    {
        ...
        static::deleting(function ($model) {
            if (in_array($model->id, AU_GUARD_ADMIN)) {
                return false;
            }
            $model->roles()->detach();
            ...
            if (self::allPermissions()->first(function ($permission) {
                ...
                foreach ($actions as $key => $action) {
                    ...
                    if (
                        ...
                        && (
                            AU_ADMIN_PREFIX . '/config/*' == $method[1]
                        || AU_ADMIN_PREFIX . '/config/update_info' == $method[1]
                        || AU_ADMIN_PREFIX . '/config' == $method[1]
                        )
                    ) {
                        return true;
