...
use Feggu\Core\Admin\Models\AdminSubscribe;
use Validator;
...
class AdminSubscribeController extends RootAdminController
{
    ...
    {
        $data = [
            'title'         => au_language_render('subscribe.admin.list'),
            'subTitle'      => '',
            'icon'          => 'fa fa-indent',
            'urlDeleteItem' => au_route_partner('admin_subscribe.delete'),
        ...
        $listTh = [
            'id' => 'ID',
            'email' => au_language_render('subscribe.admin.email'),
            'status' => au_language_render('subscribe.admin.status'),
        ...
        $arrSort = [
            ...
            'email__desc' => au_language_render('filter_sort.alpha_desc', ['alpha' =>'Email']),
            'email__asc' => au_language_render('filter_sort.alpha_asc', ['alpha' =>'Email']),
        ];
        $dataSearch = [
            'keyword'    => $keyword,
        ...
        foreach ($dataTmp as $key => $row) {
            $dataTr[] = [
                ...
                'action' => '
                    <a href="' . au_route_partner('admin_subscribe.edit', ['id' => $row['id']]) . '"><span title="' . au_language_render('action.edit') . '" type="button" class="btn btn-flat btn-sm btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
        ...
        //menuRight
        $data['menuRight'][] = '<a href="' . au_route_partner('admin_subscribe.create') . '" class="btn  btn-success  btn-flat" title="New" id="button_create_new">
        ...
        $data['urlSort'] = au_route_partner('admin_subscribe.index', request()->except(['_token', '_pjax', 'sort_order']));
        $data['optionSort'] = $optionSort;
        //=menuSort

        //menuSearch
        $data['topMenuRight'][] = '
                <form action="' . au_route_partner('admin_subscribe.index') . '" id="button_search">
                <div class="input-group input-group" style="width: 350px;">
                    <input type="text" name="keyword" class="form-control rounded-0 float-right" placeholder="' . au_language_render('subscribe.admin.search_place') . '" value="' . $keyword . '">
    ...
    {
        $data = [
            'title' => au_language_render('subscribe.admin.add_new_title'),
            'subTitle' => '',
            'title_description' => au_language_render('subscribe.admin.add_new_des'),
            'icon' => 'fa fa-plus',
            'subscribe' => [],
            'url_action' => au_route_partner('admin_subscribe.create'),
        ];
    ...
    public function postCreate()
    {
        ...
        return redirect()->route('admin_subscribe.index')->with('success', au_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        $data = [
            ...
            'icon' => 'fa fa-edit',
            'subscribe' => $subscribe,
            'url_action' => au_route_partner('admin_subscribe.edit', ['id' => $subscribe['id']]),
        ];
        return view($this->templatePathAdmin.'screen.subscribe')
            ->with($data);
    }
    ...
    public function postEdit($id)
    {
        ...
        return redirect()->route('admin_subscribe.index')
                ->with('success', au_language_render('action.edit_success'));
    }
