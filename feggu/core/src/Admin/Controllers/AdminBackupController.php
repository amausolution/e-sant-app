...
class AdminBackupController extends RootAdminController
{
    ...
    {
        $tableInfo = DB::connection(AU_CONNECTION)
            ->table('INFORMATION_SCHEMA.TABLES')
            ->select(['table_name', 'table_rows', 'data_length'])
            ->where('TABLE_SCHEMA', config('database.connections.'.AU_CONNECTION.'.database'))
        ...
        return view($this->templatePathAdmin.'screen.backup')->with(
            [
                "title"    => au_language_render('admin.backup.title'),
                "arrFiles" => $arrFiles,
                "tableInfo" => $tableInfo,
            ]
        )->render();
    ...
    public function processBackupFile()
    {
        ...
        if ($action === 'remove') {
            try {
                unlink($pathFull);
                $return = ['error' => 0, 'msg' => au_language_render('action.remove_success')];
            } catch (\Throwable $e) {
                $return = ['error' => 1, 'msg' => $e->getMessage()];
            }
        } elseif ($action === 'restore') {
            try {
                $return = Artisan::call("au:restore --path=".$file);
            } catch (\Throwable $e) {
    ...
    public function generateBackup()
    {
        ...
        $return = Artisan::call("au:backup --path=$fileName --includeTables=$includeTables --excludeTables=$excludeTables");
        return $return;
    }
}
