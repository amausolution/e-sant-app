<?php
namespace Feggu\Core\Admin\Controllers\Auth;

use Feggu\Core\Admin\Admin;
use Feggu\Core\Admin\Models\AdminPermission;
use Feggu\Core\Admin\Models\AdminRole;
...
class UsersController extends RootAdminController
{
    ...
    {
        $data = [
            'title'         => au_language_render('admin.user.list'),
            'subTitle'      => '',
            'icon'          => 'fa fa-indent',
            'urlDeleteItem' => au_route_partner('admin_user.delete'),
        ...
        $listTh = [
            'id'         => 'ID',
            'username'   => au_language_render('admin.user.user_name'),
            'name'       => au_language_render('admin.user.name'),
            'roles'      => au_language_render('admin.user.roles'),
            'permission' => au_language_render('admin.user.permission'),
        ...
        $arrSort = [
            ...
            'username__desc' => au_language_render('filter_sort.alpha_desc', ['alpha' => 'username']),
            'alpha__asc'  => au_language_render('filter_sort.alpha_asc', ['alpha' => 'username']),
        ...
        foreach ($dataTmp as $key => $row) {
            ...
            $dataTr[] = [
                ...
                'action' => '
                    <a href="' . au_route_partner('admin_user.edit', ['id' => $row['id']]) . '"><span title="' . au_language_render('action.edit') . '" type="button" class="btn btn-flat btn-sm btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
                    ' . ((Admin::user()->id == $row['id'] || in_array($row['id'], AU_GUARD_ADMIN)) ? '' : '<span onclick="deleteItem(' . $row['id'] . ');"  title="' . au_language_render('action.delete') . '" class="btn btn-flat btn-sm btn-danger"><i class="fas fa-trash-alt"></i></span>')
                ,
            ];
        }
        ...
        //menuRight
        $data['menuRight'][] = '<a href="' . au_route_partner('admin_user.create') . '" class="btn  btn-success  btn-flat" title="New" id="button_create_new">
        ...
        $data['urlSort'] = au_route_partner('admin_user.index', request()->except(['_token', '_pjax', 'sort_order']));
        ...
        //menuSearch
        $data['topMenuRight'][] = '
                <form action="' . au_route_partner('admin_user.index') . '" id="button_search">
                <div class="input-group input-group" style="width: 350px;">
                    <input type="text" name="keyword" class="form-control rounded-0 float-right" placeholder="' . au_language_render('admin.user.search_place') . '" value="' . $keyword . '">
    ...
    {
        $data = [
            'title'             => au_language_render('admin.user.add_new_title'),
            'subTitle'          => '',
            'title_description' => au_language_render('admin.user.add_new_des'),
            'icon'              => 'fa fa-plus',
            'user'              => [],
            'roles'             => $this->roles,
            'permissions'       => $this->permissions,
            'url_action'        => au_route_partner('admin_user.create'),
        ];
    ...
    public function postCreate()
    {
        ...
        return redirect()->route('admin_user.index')->with('success', au_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        $data = [
            ...
            'roles'             => $this->roles,
            'permissions'       => $this->permissions,
            'url_action'        => au_route_partner('admin_user.edit', ['id' => $user['id']]),
