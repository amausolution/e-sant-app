...
class AdminSupplierController extends RootAdminController
{
    ...
    {
        $data = [
            'title' => au_language_render('admin.supplier.list'),
            'title_action' => '<i class="fa fa-plus" aria-hidden="true"></i> ' . au_language_render('admin.supplier.add_new_title'),
            'subTitle' => '',
            'icon' => 'fa fa-indent',
            'urlDeleteItem' => au_route_partner('admin_supplier.delete'),
            'removeList' => 0, // 1 - Enable function delete list item
            'buttonRefresh' => 0, // 1 - Enable button refresh
            'buttonSort' => 0, // 1 - Enable button sort
            'css' => '',
            'js' => '',
            'url_action' => au_route_partner('admin_supplier.create'),
        ];

        $listTh = [
            'id' => 'ID',
            'name' => au_language_render('admin.supplier.name'),
            'image' => au_language_render('admin.supplier.image'),
            'email' => au_language_render('admin.supplier.email'),
            'sort' => au_language_render('admin.supplier.sort'),
        ...
        foreach ($dataTmp as $key => $row) {
            $dataTr[] = [
                'id' => $row['id'],
                'name' => $row['name'],
                'image' => au_image_render($row->getThumb(), '50px', '50px', $row['name']),
                'email' => $row['email'],
                'sort' => $row['sort'],
                'action' => '
                    <a href="' . au_route_partner('admin_supplier.edit', ['id' => $row['id']]) . '"><span title="' . au_language_render('action.edit') . '" type="button" class="btn btn-flat btn-sm btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
    ...
    public function postCreate()
    {
        ...
        $validator = Validator::make($data, [
            ...
            'name.required' => au_language_render('validation.required', ['attribute' => au_language_render('admin.supplier.name')]),
            'alias.regex' => au_language_render('admin.supplier.alias_validate'),
        ]);
        ...
        return redirect()->route('admin_supplier.index')->with('success', au_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        'css' => '',
        'js' => '',
        'url_action' => au_route_partner('admin_supplier.edit', ['id' => $supplier['id']]),
        'supplier' => $supplier,
        'id' => $id,
    ];
