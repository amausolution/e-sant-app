...
class AdminTemplateController extends RootAdminController
{
    ...
    {
        $data = [
            'title' => au_language_render('admin.template.list'),
            'subTitle' => '',
            'icon' => 'fa fa-indent',        ];

        $data["templates"] = au_get_all_template();
        $data["templatesActive"] = (new AdminTemplate)->getListTemplateActive();
        $data["templatesInstalled"] = (new AdminTemplate)->getListTemplate();
        $data["templatesUsed"] = AdminPartner::getAllTemplateUsed();
    ...
    {
        $key = request('key');
        $process = AdminPartner::first()->update(['template' => $key]);
        if ($process) {
    ...
    public function disable()
    {
        ...
        $response = ['error' => 0, 'msg' => au_language_render('action.disable_success')];
        return response()->json($response);
    }
    ...
    public function enable()
    {
        ...
        $response = ['error' => 0, 'msg' => au_language_render('action.enable_success')];
        return response()->json($response);
    }
    ...
    {
        $data =  [
            'title' => au_language_render('admin.template.import')
        ];
            ...
            if ($unzip) {
                ...
                if ($checkConfig) {
                    ...
                    if (!$configKey) {
                        ...
                        return redirect()->back()->with('error', au_language_render('admin.template.error_config'));
                    }

                    $arrTemplateLocal = au_get_all_template();
                    if (array_key_exists($configKey, $arrTemplateLocal)) {
                        File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                        return redirect()->back()->with('error', au_language_render('admin.template.error_exist'));
                    }
                    try {
                    ...
                    File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                    return redirect()->back()->with('error', au_language_render('admin.template.error_check_config'));
                }
            } else {
                return redirect()->back()->with('error', au_language_render('admin.template.error_unzip'));
            }
        } else {
            return redirect()->back()->with('error', au_language_render('admin.template.error_upload'));
        }

        if (count($config)) {
            (new AdminTemplate)->create(['key' => $config['configKey'] ?? '', 'name' => $config['name'] ?? '', 'status' => 1]);
        }
        return redirect()->route('admin_template.index')->with('success', au_language_render('admin.template.import_success'));
    }
}
