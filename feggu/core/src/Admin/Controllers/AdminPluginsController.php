...
class AdminPluginsController extends RootAdminController
{
    ...
    public function index($code)
    {
        ...
        if ($action == 'config' && $pluginKey != '') {
            $namespace = au_get_class_plugin_config($code, $pluginKey);
    ...
    protected function pluginCode($code)
    {
        ...
        if ($code == 'Laboratory') {
            $arrDefault[] = 'LaboratoryInSide';
            $arrDefault[] = 'LaboratoryExternal';
        }
        $pluginsInstalled = au_get_plugin_installed($code, $onlyActive = false);
        $plugins = au_get_all_plugin($code);
        $title = au_language_render('admin.plugin.' . $code.'_plugin');
    ...
    public function install()
    {
        $key = request('key');
        $code = request('code');
        $namespace = au_get_class_plugin_config($code, $key);
    ...
    {
        $data =  [
            'title' => au_language_render('admin.plugin.import')
        ];
    ...
    public function processImport()
    {
        ...
        $pathTmp = time();
        $linkRedirect = '';
        $pathFile = au_file_upload($data['file'], 'tmp', $pathFolder = $pathTmp)['pathFile'] ?? '';
        if ($pathFile) {
            $unzip = au_unzip(storage_path('tmp/'.$pathFile), storage_path('tmp/'.$pathTmp));
            if ($unzip) {
                ...
                if ($checkConfig) {
                    ...
                    if (!$configGroup || !$configCode || !$configKey) {
                        ...
                        return redirect()->back()->with('error', au_language_render('admin.plugin.error_config'));
                    }

                    $arrPluginLocal = au_get_all_plugin($configCode);
                    if (array_key_exists($configKey, $arrPluginLocal)) {
                        File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                        return redirect()->back()->with('error', au_language_render('admin.plugin.error_exist'));
                    }
                    ...
                    try {
                        ...
                        File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                        $namespace = au_get_class_plugin_config($configCode, $configKey);
                        $response = (new $namespace)->install();
                    ...
                    File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                    return redirect()->back()->with('error', au_language_render('admin.plugin.error_check_config'));
                }
            } else {
                return redirect()->back()->with('error', au_language_render('admin.plugin.error_unzip'));
            }
        } else {
            return redirect()->back()->with('error', au_language_render('admin.plugin.error_upload'));
        }
        if ($linkRedirect) {
            return redirect($linkRedirect)->with('success', au_language_render('admin.plugin.import_success'));
        } else {
            return redirect()->back()->with('success', au_language_render('admin.plugin.import_success'));
        }
    }
}
