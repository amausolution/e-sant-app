                        ...
                        <div  class="row">
                            ...
                            <div class="form-group col-6 col-md-3 col-lg-3">
                                ...
                                <select name="class" id="cl" class="form-control" v-model="form.classRoom">
                                    ...
                                    <option v-for="(clas, c) in classRoom" :value="c">{{ clas}}</option>
                                </select>
                            ...
                            <div class="form-group col-12 flex flex-wrap space-x-24 mt-3">
                                <div class="flex flex-row space-x-2 items-baseline">
                                    ...
                                    <input id="clim" type="checkbox" class="form-control" style="width: 1rem; height: 1rem" v-model="form.clim">
            ...
            <div class="col-12">
                    ...
                    <table class="w-full whitespace-nowrap text-base">
                        <tr v-for="(bed,k) in form.beds" :key="k" class="py-4">
                            ...
                            <td class="border-t py-3">
                                ...
                                <select  id="type" v-model="bed.type_bed"  class="form-control" >
                                    <option selected disabled value="null">{{__('Select One')}}</option>
                                    <option v-for="(tpe, b ) in typeBed" :value="b" :key="b"> {{tpe}}</option>
                                </select>
                            </td>
                            <td class="border-t py-3">
                            ...
                            <td class="border-t py-3">
                                <Button class="flex items-center px-6 py-2 text-xl font-semibold" tabindex="-1" type="button" :title="__('Remove')"
                                        @click="remove(k)"
                                        v-show="k || ( !k && form.beds.length > 1)"
                                >
                                    <i class="la la-trash text-red-400"></i>
                                </Button>
                                <Button class="flex items-center px-6 py-2 text-xl font-semibold" tabindex="-1" type="button" :title="__('Add')"
                                        @click="add(k)"
                ...
                <div class="text-right flex flex-row justify-end space-x-8 items-baseline mt-3">
                    <div class="flex flex-row space-x-3">
                        ...
                        <input id="status" type="checkbox" class="form-control" style="width: 1rem; height: 1rem" checked value="1" v-model="form.statusRoom">
                    </div>
                    <loading-button :loading="form.processing" class="btn-indigo px-3 mt-3" type="submit">{{__('Add Room')}}</loading-button>
                </div>
            </div>
        </form>
...
<script>
    ...
    export default {
        ...
        methods :{
            ...
            submitForm: function () {
                this.form.post(route('room.store'), {
                    preserveScroll: true,
                    onSuccess: () => {
                        this.form.reset();
                        // props.endPres()
