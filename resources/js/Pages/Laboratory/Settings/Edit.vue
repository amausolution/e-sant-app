            ...
            <div class="bg-white rounded-md shadow overflow-x-auto p-2">
                <form @submit.prevent="submitForm">
                    <div class="form-group">
                        ...
                        <multiselect v-model="form.analyse" :deselect-label="__('Can\'t remove this value')" track-by="id" label="title" :value="form.analyse"
                                     :placeholder="__('Select one')" :options="categories" :searchable="true" :allow-empty="false">
                        </multiselect>
                    ...
                    <div class="form-group">
                        ...
                        <multiselect v-model="form.department" :deselect-label="__('Can\'t remove this value')" track-by="id" label="department" :value="form.department"
                                     :placeholder="__('Select one')" :options="departments" :searchable="true" :allow-empty="false">
                        </multiselect>
                        <!-- <select name="" id="" v-model="form.analyse" class="form-control">
                             <option :value="category.id" v-for="(category) in categories">{{category.title}}</option>
                         </select>-->
                        <span v-if="form.errors.analyse">{{form.errors.department}}</span>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group flex flex-row space-x-4 items-baseline">
                        ...
                        <input id="status" type="checkbox" class="form-control"
                               style="width: 1rem; height: 1rem" v-model="form.status"
                               true-value="1"
                               false-value="0"
                        >
                    </div>
...
<script>
    ...
    export default {
        ...
        setup(props) {
            const form = useForm({
                analyse:props.dataAnalyse.analyse,
                price:props.dataAnalyse.price,
                take_assurance:props.dataAnalyse.take_assurance,
                status:props.dataAnalyse.status,
                duration:props.dataAnalyse.duration,
                department:props.dataAnalyse.department,
                id: props.dataAnalyse.id
            })
