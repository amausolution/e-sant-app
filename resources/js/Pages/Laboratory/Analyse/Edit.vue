<template>
    ...
    <div class="content container-fluid">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h6 class="p-4 m-b-15">Analyse <span class="text-sky-500 hover:underline">#{{analyse.slug}}</span></h6>
                    <span>{{__('Price')}}</span>
                    <span>{{typeAnalyse.price}}</span>
                    <span>{{__('Accept Assurance')}}</span>
                    <span></span>
                    <span></span>
                    <div>
                            ...
                            <div class="form-group">
                                ...
                                <div class=" flex flex-row space-x-6">
                                    <div class="flex flex-row space-x-2 items-baseline">
                                        ...
                                        <input v-model="form.result" value="0" id="negative" name="result" type="radio" class="form-control" style="width: 1rem; height: 1rem">
                                    </div>
                                    <div class="flex flex-row space-x-2 items-baseline">
                                        <label for="positive">{{ __('Positive')}}</label>
                                        <input v-model="form.result" value="1" id="positive" name="result" type="radio" class="form-control" style="width: 1rem; height: 1rem">
                                    </div>
                                </div>
                            </div>
                            <div class="w-full">
                                <div class="document-editor__editable-container border rounded-lg bg-white mb-4 ">
                                    <ckeditor v-model="form.note" :editor="editor" :config="editorConfig"  @ready="onReady" ></ckeditor>
                                </div>
                            </div>
                            <div>
